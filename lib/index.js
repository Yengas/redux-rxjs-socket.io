 ...
 * @returns {Observable<*>}
 */
function register(eventToActionMapper, actionToEventMapper, socket, rAction$, rState$, storeDispatch){
	return new Observable(() => {
		const outsideEvent$ = new Subject();
		const action$ = generateActionStreamCreator(eventToActionMapper)(socket, rState$, outsideEvent$.pipe(publishReplay(1), refCount()));
		const emit$ = generateEmitStreamCreator(actionToEventMapper)(rAction$, rState$);
 ...
 * @returns {Observable<*>}
 */
function createActionStream(eventToActionMapper, actionToEventMapper, socket, rAction$, rState$){
